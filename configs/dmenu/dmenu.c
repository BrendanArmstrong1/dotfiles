...
keypress(XKeyEvent *ev)
{
	...
	if (ev->state & ControlMask) {
		...
		case XK_j: // Ctrl+j: move down (next item)
        if (sel && sel->right && (sel = sel->right) == next) {
          curr = next;
          calcoffsets();
          drawmenu();
        ...
        break;
		case XK_k: // Ctrl+k: move up (previous item)
        if (sel && sel->left && (sel = sel->left)->right == curr) {
          curr = prev;
          calcoffsets();
          drawmenu();
...
motionevent(XButtonEvent *ev)
{
  struct item *it;
	int x = 0, y = 0, w, h = bh;
	unsigned int row, col, item_index;
	...
	if (lines > 0) {
		...
		w = (mw - x) / (columns > 0 ? columns : 1);

		col = (ev->x - x) / w;
		row = (ev->y - y - bh) / h;

		if (col < 0 || row < 0 || (columns > 0 && col >= columns) || row >= lines)
			return;

		item_index = col * lines + row;

		it = curr;
		while (it && item_index--)
			it = it->right;
	} else {
		/* Horizontal list (single line, linear scroll) */
		x += inputw;
		w = TEXTW("<");
		if (!prev || !curr->left)
			w = 0;
